...
function PDFExtractPagesTool() {
  const [pdfFile, setPdfFile] = useState(null);
  const [allPages, setAllPages] = useState([]);
  const [selectedPages, setSelectedPages] = useState(new Set()); // Pages to EXTRACT
  ...
  // Toggle page selection (mark for extraction)
  ...
  const selectRange = () => {
    ...
    const selectedNums = allPages
      .filter(p => selectedPages.has(p.id))
      .map(p => p.pageNum);

    const minPage = Math.min(...selectedNums);
    const maxPage = Math.max(...selectedNums);

    const newSet = new Set();
    allPages.forEach(p => {
      if (p.pageNum >= minPage && p.pageNum <= maxPage) {
        newSet.add(p.id);
      }
    });
  ...
  const handleExtractPages = async () => {
    ...
    const saveResult = await window.electronAPI.saveFile({
      defaultPath: pdfFile.name.replace('.pdf', '_extracted.pdf'),
      filters: [{ name: 'PDF Files', extensions: ['pdf'] }]
    });
    ...
    try {
      // Get page numbers to extract (1-indexed)
      const pagesToExtract = allPages
        .filter(p => selectedPages.has(p.id))
        .map(p => p.pageNum);
      ...
      if (result?.success) {
        ...
        alert(`${pagesToExtract.length} pages extracted successfully!`);
      } else {
  ...
  return (
    <div className="pdf-extract-tool">
          ...
          {allPages.length > 0 && (
            <>
              ...
              {selectedPages.size >= 2 && (
                <button className="toolbar-btn" onClick={selectRange} title="Select Range">
                  ↔️ Range
                </button>
              )}
            </>
      ...
      {!isLoading && !pdfFile && (
        <div className="empty-state">
          ...
          <h3>Extract Pages</h3>
          <p>Select a PDF file, mark pages to extract, and create a new PDF.</p>
      ...
      {!isLoading && allPages.length > 0 && (
        <div className="pages-grid">
          {allPages.map((page) => (
            <div
              key={page.id}
              className={`page-item ${selectedPages.has(page.id) ? 'marked-extract' : ''}`}
      ...
      {allPages.length > 0 && (
        <div className="extract-footer">
          <div className="extract-summary">
            ...
            <span className="summary-item extract">
              <strong>{selectedPages.size}</strong> to extract
            </span>
          </div>
...
export default PDFExtractPagesTool;
